...
export function createProgram(gl, shaders) {
    ...
    let addToObject = (object, path, value) => {
        ...
        for (const [, part, , indexstr] of matches) {
            if (indexstr) {
                ...
                currentObject[index] ?? (currentObject[index] = {});
...
export function createTexture(gl, { texture = gl.createTexture(), unit, target = gl.TEXTURE_2D, level = 0, iformat = gl.RGBA, format = gl.RGBA, type = gl.UNSIGNED_BYTE, image, data, width, height, depth, wrapS, wrapT, wrapR, min, mag, mip, }) {
    if (unit != null) {
...
export function createBuffer(gl, { buffer = gl.createBuffer(), target = gl.ARRAY_BUFFER | gl.ELEMENT_ARRAY_BUFFER, hint = gl.STATIC_DRAW, data, }) {
    gl.bindBuffer(target, buffer);
    gl.bufferData(target, data, hint);
    return buffer;
}
export function configureAttribute(gl, { location, count, type, normalize = false, stride = 0, offset = 0, divisor = 0, }) {
    gl.enableVertexAttribArray(location);
    gl.vertexAttribPointer(location, count, type, normalize, stride, offset);
    gl.vertexAttribDivisor(location, divisor);
}
export function createSampler(gl, { sampler = gl.createSampler(), wrapS, wrapT, wrapR, min, mag, }) {
    if (wrapS) {
