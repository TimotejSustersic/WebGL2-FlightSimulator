...
export class ThirdPersonController extends Controller {
    constructor(node, canvasElement, { pitch = 1.4, yaw = Math.PI, distance = 25, moveSensitivity = 0.002, zoomSensitivity = 0.001, } = {}) {
        super();
        this.distanceMax = 50;
        this.distanceMin = 10;
        this.disable = true;
        this.toggleFlag = false;
        this.interpolationSpeed = 4.5;
        this.node = node;
    ...
    update(time, dt) {
        if (this.disable)
            return;
        if (this.toggleView(time))
            return;
        const transform = this.node.getComponentOfType(Transform);
        if (!transform)
            return;
        const targetTransform = this.interactionNode.getComponentOfType(Transform);
        if (!targetTransform)
            return;
...
//# sourceMappingURL=ThirdPersonController.js.map