...
import { Mesh } from './common/engine/core/Mesh.js';
import { getGlobalModelMatrix, getGlobalViewMatrix, getLocalViewMatrix, getProjectionMatrix } from './common/engine/core/SceneUtils.js';
...
export class Renderer {
    ...
    getViewProjectionMatrix(camera) {
        const vpMatrix = getGlobalModelMatrix(camera);
        mat4.invert(vpMatrix, vpMatrix);
        mat4.mul(vpMatrix, getProjectionMatrix(camera), vpMatrix);
        return vpMatrix;
    }
    render(scene, camera) {
        const gl = this.gl;
    ...
    renderNode(node, mvpMatrix) {
        ...
        mat4.mul(mvpMatrix, mvpMatrix, getLocalViewMatrix(node));
        if (node.getComponentsOfType(Mesh)) {
            gl.uniformMatrix4fv(uniforms.uModelMatrix, false, mvpMatrix);
            for (const primitive of node.getComponentsOfType(Mesh).primitives) {
