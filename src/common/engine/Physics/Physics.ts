...
import { Interaction, Vehicle } from '../core/Interaction.js';
...
export class Physics {
    ...
    scene: Node;
    document: Document;
    interactContainer: HTMLElement;

    constructor(scene: Node, canvas: HTMLCanvasElement) {
        this.scene = scene;
    ...
    aabbAdjacent(aabb1, aabb2, margin: number) {
    ...
    resolveAABBCollision(a: Node, b: Node, time: number): void {
        // Get global space AABBs.
        const aBox = this.getTransformedAABB(a);
        const bBox = this.getTransformedAABB(b);
      
        let interaction: Interaction = b.getComponentOfType(Interaction);
        ...
        if (interaction != undefined) {
            if (interaction.checkCollision) {
                ...
                if (isClose) {
                    ...
                    const aFirstPersonController: FirstPersonController = a.getComponentOfType(FirstPersonController);
    ...
    resolveCircularCollision(a: Node, b: Node, time: number): void {        
        ...
        const d3D = Math.sqrt(
            ((bTranslation[0] - aTranslation[0]) ** 2) + 
        ...
        if (d3D <= a.r + b.r) {
            ...
            if (d2D + a.r > b.r) {
                ...
                let planeInstance: Plane = a.getComponentOfType(Plane);
