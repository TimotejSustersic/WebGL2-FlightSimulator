...
class Main extends Application {
    ...
    async start(resolve: Function): Promise<void> {
        ...
        if (!this.scene) throw new Error('Scene did not load.');

        this.camera = this.scene.find(node => node.getComponentOfType(Camera));
        if (!this.camera) throw new Error('There is no camera in the scene.');
